...
describe('which canvases a note has claimed (#46)', () => {
  ...
  test('a canvas nobody owns is not free for a note to take', () => {
    const nobodys = withLastCanvas(EMPTY_INDEX, 'c-1', null);
  ...
  test("an older build's last canvas is still adopted by the first note to ask", () => {
    ...
    expect(upgraded.unowned).toEqual([]);
  ...
  test('a note taking a canvas nobody owned makes it that note s, and no longer nobody s', () => {
    ...
    expect(claimed.unowned).toEqual([]);
    expect(belongsToANote(claimed, 'c-1')).toBe(true);
  });

  test('a retired id belongs to nobody either, so the next note may have it', () => {
    const nobodys = withLastCanvas(EMPTY_INDEX, 'default', null);
    expect(withoutCanvas(nobodys, 'default').unowned).toEqual([]);
  ...
  test('what nobody owns survives a save and load, and rubbish in it is dropped', () => {
    const saved = serializeCanvasIndex(withLastCanvas(EMPTY_INDEX, 'c-1', null));
    expect(parseCanvasIndex(saved).unowned).toEqual(['c-1']);
    expect(parseCanvasIndex(JSON.stringify({unowned: ['c-1', 'not a canvas', 7, null]})).unowned).toEqual(['c-1']);
    expect(parseCanvasIndex(JSON.stringify({unowned: 'nope'})).unowned).toEqual([]);
  });
