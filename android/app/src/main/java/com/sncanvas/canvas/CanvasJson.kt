 ...
 * (`{"version":1,"elements":[...]}`) written and read by [CanvasModule]'s
 * saveCanvas/loadCanvas. The JSON itself is [JsonParser]/[JsonObjectWriter]
 * in Json.kt, hand-rolled so this stays plain-JUnit testable; what is written
 * here is only the schema: one reader and one writer per nested type.
 */
object CanvasJson {
     ...
     * e.g. sn-shapes' favoritesStorage.ts) for `CanvasModule.saveCanvas`.
     *
     ...
     * element list for `CanvasModule.loadCanvas`. Never throws: malformed
