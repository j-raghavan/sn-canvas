...
/**
 * The SuperCanvas surface (spec/SuperCanvas-PRD.md §9, NFR1-NFR3, NFR5): it
 ...
 * [registry], which is how [SuperCanvasModule] reaches the live canvas.
...
class SuperCanvasView(
    ...
    val controller =
        CanvasController(
            ...
            object : CanvasController.Listener {
                ...
                override fun onUiState(uiState: CanvasUiState) = events.onUiState(this@SuperCanvasView, uiState)
            ...
            object : ScaleGestureDetector.SimpleOnScaleGestureListener() {
                override fun onScale(detector: ScaleGestureDetector): Boolean {
                    ...
                    controller.setViewport(
                        SuperCanvasCore.zoomTo(controller.state, detector.scaleFactor.toDouble(), focal.x, focal.y).transform,
                    )
                    showMinimap()
                    return true
                }
    ...
    fun zoomTo100() {
        ...
        controller.setViewport(SuperCanvasCore.zoomTo(controller.state, 1.0 / controller.state.zoom, center.x, center.y).transform)
    }
    ...
    private fun eraseAt(
        ...
        SuperCanvasCore.hitsAt(world.x, world.y, controller.state.elements).mapTo(erase.ids) { it.id }
        invalidate()
    }
    ...
    private fun handleMove(
        ...
        when (val active = gesture) {
            CanvasGesture.Pan -> {
                controller.setViewport(SuperCanvasCore.panBy(state, dx, dy).transform)
    ...
    private fun tapSelect(
        ...
        if (isTap) controller.select(SuperCanvasCore.hitTest(at.x, at.y, controller.state.elements)?.id)
    }
    ...
    private fun commitDrawnShape(
        ...
        controller.insert(
            when {
                ...
                CanvasTools.isConnector(toolMode) -> SuperCanvasCore.connectorFromDrag(id, toolMode, start, end, controller.state.elements)
                else -> SuperCanvasCore.boxFromDrag(id, toolMode, start, end)
            },
        )
    }
