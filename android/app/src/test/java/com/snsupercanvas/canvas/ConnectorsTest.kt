...
class ConnectorsTest {
    ...
    fun `connectorFromDrag binds each end to the shape it was drawn from or to`() {
        val arrow = SuperCanvasCore.connectorFromDrag("n", "arrow", Point(5.0, 5.0), Point(110.0, 110.0), listOf(boxA, boxB))
    ...
    fun `connectorFromDrag leaves an end drawn on empty canvas unbound`() {
        val intoShape = SuperCanvasCore.connectorFromDrag("n", "line", Point(50.0, 50.0), Point(5.0, 5.0), listOf(boxA))
        assertNull(intoShape.startElementId)
        assertEquals("boxA", intoShape.endElementId)
        val outOfShape = SuperCanvasCore.connectorFromDrag("n", "line", Point(5.0, 5.0), Point(50.0, 50.0), listOf(boxA))
    ...
    fun `bindingTargetAt never binds to another connector, even one drawn on top`() {
        ...
        assertEquals("boxA", SuperCanvasCore.bindingTargetAt(Point(5.0, 5.0), listOf(boxA, over))?.id)
    }
}
