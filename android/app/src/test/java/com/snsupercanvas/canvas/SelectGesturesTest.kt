...
class SelectGesturesTest {
    ...
    fun `a corner handle resizes, the rotate handle rotates, and a connector's end re-points it`() {
        ...
        val rotate = SuperCanvasCore.rotateHandlePoint(box, 1.0)
    ...
    fun `each drag's edit is what its release commits`() {
        ...
        assertEquals(
            controller.fitted(SuperCanvasCore.resizeElement(state, "box", Corner.BOTTOM_RIGHT, 150.0, 150.0), "box"),
            edited(CanvasGesture.Resize(Corner.BOTTOM_RIGHT), "box", Point(150.0, 150.0)),
        )
        assertEquals(
            SuperCanvasCore.rotateElement(state, "box", Point(150.0, 50.0)),
            edited(CanvasGesture.Rotate, "box", Point(150.0, 50.0)),
        )
        assertEquals(
            SuperCanvasCore.moveEndpoint(state, "line", Endpoint.END, Point(420.0, 30.0), null),
            edited(CanvasGesture.DragEndpoint(Endpoint.END), "line", Point(420.0, 30.0)),
        )
        // Dropped on the box, the line's end binds to it.
        assertEquals(
            SuperCanvasCore.moveEndpoint(state, "line", Endpoint.END, Point(50.0, 50.0), "box"),
